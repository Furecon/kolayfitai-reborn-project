...
export function SocialAuthButtons({ mode }: SocialAuthButtonsProps) {
  ...
  const buttonText = mode === 'login' ? 'ile Giriş Yap' : 'ile Kayıt Ol'
  ...
  return (
    <div className="space-y-3">
      ...
      >
        ...
        {loadingProvider === 'google' ? 'Yükleniyor...' : `Google ${buttonText}`}
        {isNative && <span className="text-xs text-gray-500 ml-2">(Native)</span>}
      </Button>
      ...
      >
        ...
        {loadingProvider === 'facebook' ? 'Yükleniyor...' : `Facebook ${buttonText}`}
        {isNative && <span className="text-xs text-gray-500 ml-2">(Native)</span>}
      </Button>
      ...
      >
        ...
        {loadingProvider === 'apple' ? 'Yükleniyor...' : `Apple ${buttonText}`}
        {isNative && <span className="text-xs text-gray-500 ml-2">(Native)</span>}
      </Button>
    </div>
  )
