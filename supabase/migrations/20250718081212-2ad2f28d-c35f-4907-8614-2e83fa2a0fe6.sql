...
('Döner Kebap', 'Doner Kebab', 250, 25, 8, 15, 1, 'Et Yemekleri', true),
('Lahmacun', 'Turkish Pizza', 280, 12, 35, 12, 2, 'Hamur İşleri', true),
('Pilav', 'Rice Pilaf', 165, 3, 35, 1, 1, 'Tahıl', true),
('Köfte', 'Turkish Meatballs', 220, 18, 5, 15, 1, 'Et Yemekleri', true),
('Baklava', 'Baklava', 520, 9, 45, 35, 3, 'Tatlılar', true),
('Çorba', 'Turkish Soup', 85, 4, 12, 3, 2, 'Çorbalar', true),
('Dolma', 'Stuffed Grape Leaves', 180, 6, 25, 7, 4, 'Sebze Yemekleri', true),
('Meze', 'Turkish Appetizers', 150, 8, 15, 8, 3, 'Mezeler', true),
('Börek', 'Turkish Pastry', 320, 12, 28, 18, 2, 'Hamur İşleri', true),
('Kebap', 'Grilled Meat', 280, 26, 2, 18, 0, 'Et Yemekleri', true),
...
-- Create storage policy for meal photos
CREATE POLICY "Users can upload their meal photos" ON storage.objects
  FOR INSERT WITH CHECK (
    bucket_id = 'meal-photos' AND 
    auth.uid()::text = (storage.foldername(name))[1]
  );

CREATE POLICY "Users can view their meal photos" ON storage.objects
  FOR SELECT USING (
    bucket_id = 'meal-photos' AND 
    auth.uid()::text = (storage.foldername(name))[1]
  );
